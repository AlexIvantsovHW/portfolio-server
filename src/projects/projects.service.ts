...
export class ProjectsService {
  ...
  async findAll():Promise<ProjectEntity[]> {
    ...
    const dateTransformProjects=projects.map((project,idx)=>{return({...project,startAt:project.startAt.toString(),endAt:project.endAt.toString()})})
     return dateTransformProjects
  }
  async findOne(id:number):Promise<IProjects|{message:string}>{
   const project= await this.prisma.projects.findUnique({where:{id:id}})
   if(!project){
    return {message:`Project with id ${id} doesn't exist in DB`}
   }
    const dateTransformProject={...project,startAt:project?.startAt.toString(),endAt:project?.endAt.toString()}
